    ...
    return(
        <>
            <div style={todo.complete?{backgroundColor:"green",color:"black"}:{}} className="todo">
                ...
                <div>
                    <button onClick={() => dispatch({type:ACTIONS.TOGGLE_TODO,payload:{id:todo.id}})}><TiTickOutline/></button>
                    <button onClick={() => dispatch({type:ACTIONS.DELETE_TODO,payload:{id:todo.id}})}><MdOutlineDeleteForever/></button>
                    <button onClick={() => setChange(!change)}><BiEdit/></button>
                </div>
            </div>
            <div className={`change_outer ${change?"show":""}`}>
                <div className="change_inner">
                    <div onClick={()=>setChange(!change)} className="exit"><IoCloseCircleSharp/></div>
